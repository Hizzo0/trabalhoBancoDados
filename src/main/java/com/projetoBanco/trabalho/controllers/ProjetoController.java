...
public class ProjetoController {
    ...
    //verifica se existe um docente com o cpf passado, se existir, associa o docente como coordenador do projeto
    @PostMapping
    public ResponseEntity<?> criarProjeto(@RequestBody ProjetoDTO dto) {
        ...
        if (dto.getAgenciaFinanciador() != null && !dto.getAgenciaFinanciador().isEmpty()) {
            ...
            if (finOpt.isPresent()) {
                ...
                // Como é OneToOne, verificamos se esse financiamento já não está em outro projeto
                if (financiamentoExistente.getProjeto() != null && !financiamentoExistente.getProjeto().getId().equals(projetoSalvo.getId())) {
